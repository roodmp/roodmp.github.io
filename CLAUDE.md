...
- Format Translator (format-translator.html) — WYSIWYG editor that translates formatting to Slack/Discord/Reddit/Markdown syntax
...
├── mcp-server/                 # About Me MCP server (Vercel-deployed)
...
**Key Files:**
- `index.html` - Resume/CV page, the actual homepage served by GitHub Pages
- `about.html` - About/Workspace page (project browser with board/list views)
- `main.js` - Core JavaScript for navigation and mobile menu
...
- `.github/workflows/pages.yml` - GitHub Actions deployment workflow (deploys the raw repo directly; no Jekyll build, so no `_config.yml`)
...
- Modular functions for mobile menu and page interactions
...
- Each entry needs: `id` (next sequential MR-### number), `name`, `status`, `type`, `tags`, `description`, `link` (if navigable), and `image` (if available)

**Projects Array Schema:**
```javascript
{
  id: 'MR-026',            // String — sequential ID, next available: MR-026
...
- `powershell.html` is a standalone quiz page with no navigation (not even a back link)
...
4. Verify mobile menu opens/closes correctly on all pages
...
3. **Update navigation on the 3 main pages** if adding the page to the top nav bar (see Navigation Update Checklist above). Most tool/game pages do NOT go in the top nav.
